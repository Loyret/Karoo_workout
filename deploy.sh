...
# Nginx должен читать static/ и templates/
usermod -aG karoo www-data 2>/dev/null || true
print_ok "www-data добавлен в группу karoo"
...
print_ok "Права на файлы установлены"
...
# Почта (SMTP) — настраивается через админ-панель: /admin/settings
MAIL_ENABLED=false
...
echo ""
echo -e "  ${YELLOW}═══ Настройка почты ═══${NC}"
echo "    Почта отключена по умолчанию."
echo "    Настройте через админ-панель: https://${DOMAIN}/admin/settings"
echo ""
